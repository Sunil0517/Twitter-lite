...
const main = async () => {
  ...
  $("section >div > div > div").each((_, el) => {
    if (productdata.length < 50) {
      productdata.push({
        ...
        // email: $(
        //   "div > div.css-1dbjc4n.r-1wbh5a2.r-dnmrzs > a",
        //   el
        // ).text(),
        image: $(
          "div.r-1p0dtai.r-1pi2tsx.r-1d2f490.r-u8s1d.r-ipm5af.r-13qz1uu > div > div",
          el
        ).attr("src"),
        // image: $(
  ...
  await writeFile("tweets.json", JSON.stringify(productdata));
  await browser.close();
};
main();
